        ...
        for i in range(num_branches if self.multi_scale_output else 1):
            ...
            for j in range(num_branches):
                if j > i:
                    ...
                    fuse_layer.append(nn.ReLU(True))
                else:
                    conv3x3s = []
                    for k in range(i-j):
                        if k == i - j - 1:
...
class PoseHigherResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.conv_scale_layers = self._make_conv_scale_layers(cfg)
    ...
    def _make_conv_scale_layers(self, cfg):
        extra = cfg.MODEL.EXTRA

        conv_scale_layers = []

        input_channels = extra.STAGE4.NUM_CHANNELS[0]
        conv_scale_layers.append(
            nn.Sequential(
                nn.Conv2d(input_channels, cfg.MODEL.NUM_JOINTS, 1, 1, 0),
                nn.BatchNorm2d(cfg.MODEL.NUM_JOINTS), nn.ReLU(True),
                nn.Conv2d(
                    cfg.MODEL.NUM_JOINTS, 
                    cfg.MODEL.NUM_JOINTS, 9, 1, 4, 
                    groups=cfg.MODEL.NUM_JOINTS
                    ),
            )
        )
        for i in range(extra.DECONV.NUM_DECONVS):
            input_channels = extra.DECONV.NUM_CHANNELS[i]
            conv_scale_layers.append(
                nn.Sequential(
                nn.Conv2d(input_channels, cfg.MODEL.NUM_JOINTS, 1, 1, 0),
                nn.BatchNorm2d(cfg.MODEL.NUM_JOINTS), nn.ReLU(True),
        ...
        return nn.ModuleList(conv_scale_layers)
        ...
        for i in range(num_branches_cur):
            if i < num_branches_pre:
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    ...
                    transition_layers.append(nn.ReLU(True))
            else:
                conv3x3s = []
    ...
    def forward(self, x):
        ...
        for i in range(self.stage2_cfg['NUM_BRANCHES']):
            if not isinstance(self.transition1[i], nn.ReLU):
        ...
        for i in range(self.stage3_cfg['NUM_BRANCHES']):
            if not isinstance(self.transition2[i], nn.ReLU):
        ...
        for i in range(self.stage4_cfg['NUM_BRANCHES']):
            if not isinstance(self.transition3[i], nn.ReLU):
        ...
        scale = self.conv_scale_layers[0](x)
        # print(torch.max(scale), torch.min(scale))
        y = torch.cat([scale, y], 1)
        final_outputs.append(y)

        for i in range(self.num_deconvs):
            if self.deconv_config.CAT_OUTPUT[i]:
                x = torch.cat((x, y[:,self.cfg.MODEL.NUM_JOINTS:]), 1)

            x = self.deconv_layers[i](x)
            scale = self.conv_scale_layers[i+1](x)
            # print(torch.max(scale), torch.min(scale))
    ...
    def init_weights(self, pretrained='', verbose=True):
        ...
        for layer in self.conv_scale_layers:
