...
class MultiLossFactory(nn.Module):
    def __init__(self, cfg):
        ...
        self.ae_loss = nn.ModuleList(
            [
                AELoss(cfg.LOSS.AE_LOSS_TYPE) 
                if with_ae_loss else None
    ...
    def forward(self, outputs, heatmaps, masks, joints):
        ...
        for idx in range(len(outputs)):
            ...
            if self.heatmaps_loss[idx]:
                scale_pred = outputs[idx][:,:self.num_joints] * (heatmaps[idx] > 0).float()

                scale_loss = self.scale_loss[idx](masks[idx], scale_pred)
                scale_losses.append(scale_loss)

                heatmaps_pred = outputs[idx][:, self.num_joints:self.num_joints*2]

                heatmaps_loss = self.heatmaps_loss[idx](
                    heatmaps_pred, heatmaps[idx], masks[idx], scale_pred
                )
                heatmaps_loss = heatmaps_loss * self.heatmaps_loss_factor[idx]
                heatmaps_losses.append(heatmaps_loss)

                offset_feat = self.num_joints*2
            else:
        ...
        return heatmaps_losses, scale_losses, push_losses, pull_losses
    ...
    def _forward_check(self, outputs, heatmaps, masks, joints):
        ...
        assert isinstance(heatmaps, (list, tuple)), \
        ...
        assert isinstance(joints, (list, tuple)), \
